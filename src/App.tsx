...
export default function App() {
  ...
  const handleNavigate = (view: string, resetScroll = true) => {
    ...
    else if (view === "policy-mua-hang")
      path = "/chinh-sach/huong-dan-mua-hang";
  ...
  const renderContent = () => {
    ...
    switch (currentView) {
      case "admin":
        ...
        return (
          <AdminPanel
            ...
            onRefreshProducts={() => fetchAndMergeProducts(true)}
          />
        );
      case "categories":
        return (
      ...
      case "policy-van-chuyen":
        return (
          <PolicyView activeTab="van-chuyen" onNavigate={handleNavigate} />
        );
      case "policy-doi-tra":
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-12">
                  {[
                    ...
                    },
                    {
                      ...
                      desc: "Nhận hàng thanh toán",
                    },
                    {
                      icon: Clock,
                      title: "Đổi trả dễ dàng",
                      desc: "Miễn phí nếu hàng lỗi do NSX",
                    },
                    {
                      icon: Headphones,
                      title: "Hỗ trợ kỹ thuật",
                      desc: "Tư vấn lắp đặt miễn phí",
                    },
                  ].map((item, idx) => (
                    <div
                      key={idx}
                      className="flex flex-col items-center sm:flex-row gap-4 text-center sm:text-left"
                    >
                      <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-brand-primary shadow-sm">
                        <item.icon size={24} />
                      </div>
                      <div>
                        <h4 className="font-bold text-slate-800 text-sm not-italic font-sans">
                          {item.title}
                        </h4>
              ...
              <div className="bg-brand-secondary rounded-3xl p-8 md:p-12 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="relative z-10 max-w-xl text-center md:text-left">
                  ...
                  <p className="text-blue-100 mb-8 text-lg">
                    Để lại thông tin số điện thoại, đội ngũ kỹ thuật của chúng
                    tôi sẽ liên hệ tư vấn và khảo sát trực tiếp khu vườn của
                    Chú/Bác.
                  </p>
                  <button
